...
@section('content')
    ...
    <main class="offset-md-3 offset-lg-2 col-12 ms-sm-auto px-md-4 py-4" style="height:100vh; overflow-y:auto;">
        ...
        @endif
        <h3 class="fw-normal pb-3" style=" color: #3b82f6;">Order List - Cafe Pixel</h3>
                    ...
                    <table class="table table-bordered table-hover table-striped align-middle text-center">
                        ...
                        @forelse($orders as $o)
                            ...
                            >
                                ...
                                @if(auth()->user()->getrole() == "admin" || auth()->user()->getrole() == "cashier")
                                    <td>
                                        <a href="{{route('order.vieweach',$o->id)}}" class="view-order-btn text-primary me-2" title="View">
                                            <i class="bi bi-eye fs-5"></i>
                                        </a>
                                        <a href="{{route('order.edit',$o->id)}}" class="text-success me-2"><i class="bi bi-pencil fs-5"></i></a>
                                        <a href="{{route('item.realpay',$o->id)}}" class="view-order-btn text-success me-2" title="Pay Now">
                                            <i class="bi bi-credit-card" aria-label="Pay Now"></i>
                                        </a>
                                        <a href="{{route('item.laterrealpay',$o->id)}}" class="view-order-btn text-info me-2" title="Add to Bill">
                                            <i class="bi bi-plus-circle" aria-label="Add to Bill"></i>
                                        </a>
                                        <form action="{{route('order.delete',$o->id)}}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            <button type="submit" class="btn btn-link p-0 text-danger"><i class="bi bi-trash fs-5"></i></button>
                                        </form>
                                    </td>
                                @elseif(auth()->user()->getrole() == "designer" && $o->state == "placed")
                                    <td>
                                        <a href="{{route('order.vieweach',$o->id)}}" class="view-order-btn text-primary me-2" title="View">
                                            Finished
                                        </a>
                                    </td>
                                @elseif(auth()->user()->getrole() == "printer" && $o->state == "designed")
                                    <td>
                                        <a href="{{route('order.vieweach',$o->id)}}" class="view-order-btn text-primary me-2" title="View">
                                            Finished
                                        </a>
